...
def solve_h_captcha(driver: webdriver, url: str, sitekey, applicant: spainApplicants.Applicant):
    ...
    waitByMethods.wait_visibility_by_name(driver, h_captcha_response_name)
...
def refresh_page_before_bls_captcha_is_working(driver: webdriver, applicant: spainApplicants.Applicant):
    ...
    while time.time() - starting_time < timeout:
        try:
            WebDriverWait(driver, WAIT_TIMEOUT).until(ec.visibility_of_element_located(
...
def main():
    ...
    applicants_normal = [spainApplicants.mariia, spainApplicants.nadezhda, spainApplicants.anzorov, spainApplicants.katerina]
    # applicants_premium = [spainApplicants.mariia, spainApplicants.nadezhda]

    while True:
        try:
            multiprocessing_applicants_start(applicants_normal, 'Normal')
